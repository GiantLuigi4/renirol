package tfc.renirol.frontend.enums.masks;

import org.lwjgl.vulkan.VK13;

public enum StageMask {
    GRAPHICS(VK13.VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT),
    DRAW(
            VK13.VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
                    VK13.VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT |
                    VK13.VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT |
                    VK13.VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT |
                    VK13.VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
                    VK13.VK_PIPELINE_STAGE_VERTEX_SHADER_BIT |
                    VK13.VK_PIPELINE_STAGE_VERTEX_INPUT_BIT |
                    VK13.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT |
                    VK13.VK_PIPELINE_STAGE_HOST_BIT |
                    VK13.VK_PIPELINE_STAGE_TRANSFER_BIT
    ),
    TRANSFER(VK13.VK_PIPELINE_STAGE_TRANSFER_BIT),
    TOP_OF_PIPE(VK13.VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT),
    BOTTOM_OF_PIPE(VK13.VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT),
    HOST(VK13.VK_PIPELINE_STAGE_HOST_BIT),
    EARLY_FRAGMENT_TEST(VK13.VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT),
    FRAGMENT_TEST(VK13.VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK13.VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT),
    COLOR_ATTACHMENT_OUTPUT(VK13.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT),
    DEPTH_ATTACHMENT_OUTPUT(VK13.VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    public final int value;

    StageMask(int value) {
        this.value = value;
    }
}
